...
# --- 1. Defensive Config Loading ---
try:
    with open('config/tickers.json', 'r') as f: tickers = json.load(f)
except Exception:
    tickers = {"demand": [], "bottleneck": [], "networking_optical": [], "power_cooling": [], "stress": []}
...
norm_config = weights.get('normalization', {})
max_return_clip = norm_config.get('max_return_clip', 0.15)
min_return_clip = norm_config.get('min_return_clip', -0.15)

rot_config = weights.get('rotation_normalization', {})
rotation_clip = rot_config.get('max_return_clip', 0.30)

thresholds = weights.get('thresholds', {'high': 60, 'low': 40})
h, l = thresholds.get('high', 60), thresholds.get('low', 40)

target_vectors = weights.get('target_vectors', {
    "A": { "name": "True Bottleneck Still Tight", "demand": 75, "bottleneck": 75, "substitution": 50, "stress": 25, "breadth": 75 },
    "B": { "name": "Bottleneck Easing, ROI Holding", "demand": 60, "bottleneck": 45, "substitution": 75, "stress": 35, "breadth": 65 },
    "C": { "name": "Hardware Late-Cycle Squeeze", "demand": 40, "bottleneck": 75, "substitution": 50, "stress": 75, "breadth": 40 },
    "D": { "name": "CAPEX Bubble Fear", "demand": 30, "bottleneck": 30, "substitution": 60, "stress": 80, "breadth": 30 }
})

# --- 2. Defensive Data Loading ---
try:
    with open('data/warnings.json', 'r') as f: warnings_list = json.load(f)
except Exception:
    warnings_list = []
...
try:
    ...
    if df.empty:
        raise ValueError("raw_prices.csv is empty")
    ...
    print(f"Error reading raw_prices.csv: {e}")
    pivot_df = pd.DataFrame()
    latest_date_obj = datetime.now(timezone.utc)
    latest_date = latest_date_obj.strftime('%Y-%m-%d')

# Utility Functions
def get_return(ticker, days):
    if ticker not in pivot_df.columns: return 0.0
...
def get_basket_return(basket, days):
    returns = [get_return(t, days) for t in basket if t in pivot_df.columns and len(pivot_df[t].dropna()) >= days + 1]
    return float(np.mean(returns)) if returns else 0.0

def normalize(val, scale=None):
    clip_val = scale if scale else max_return_clip
...
demand_20 = get_basket_return(tickers.get('demand', []), 20) - qqq_20d
demand_60 = get_basket_return(tickers.get('demand', []), 60) - get_return('QQQ', 60)
demand_120 = get_basket_return(tickers.get('demand', []), 120) - get_return('QQQ', 120)
demand_score = normalize((demand_20 * 0.4) + (demand_60 * 0.4) + (demand_120 * 0.2))

bot_20 = get_basket_return(tickers.get('bottleneck', []), 20) - smh_20d
bot_60 = get_basket_return(tickers.get('bottleneck', []), 60) - get_return('SMH', 60)
bot_120 = get_basket_return(tickers.get('bottleneck', []), 120) - get_return('SMH', 120)
...
stress_underperf = get_basket_return(tickers.get('stress', []), 20) - qqq_20d
norm_underperf = max(0, min(100, 50 - (stress_underperf / max_return_clip) * 50))

drawdowns = []
for t in tickers.get('stress', []):
    if t in pivot_df.columns:
        series = pivot_df[t].dropna()
        if len(series) >= 60:
            dd = float((series.iloc[-1] / series.tail(60).max()) - 1)
            drawdowns.append(dd)
...
stress_score = float((norm_underperf * 0.50) + (norm_dd * 0.35) + (norm_credit * 0.15))

universe = set(tickers.get('demand', []) + tickers.get('bottleneck', []) + tickers.get('networking_optical', []) + tickers.get('power_cooling', []) + tickers.get('stress', []))
valid_tickers = 0
conditions_true = 0
...
for t in universe:
    if t in pivot_df.columns:
        ...
        if len(series) >= 50:
            ...
            if p > series.rolling(20).mean().iloc[-1]: conditions_true += 1
            if p > series.rolling(50).mean().iloc[-1]: conditions_true += 1
            if ret_20 > qqq_20d: conditions_true += 1
            if ret_20 > smh_20d: conditions_true += 1

conditions_total = valid_tickers * 4
breadth_score = float((conditions_true / conditions_total) * 100) if conditions_total > 0 else 50.0
...
if closest_code == "B":
    ...
    watch_list.extend([
        "If Demand Score rises above 60 while Stress stays low, the regime shifts toward a broad, confirmed bull phase (A).",
        "If Rotation stays very high but Demand fails to improve, ASIC/networking may become a crowded sub-theme rather than a broad AI confirmation.",
        "If Stress rises above 60 while Bottleneck remains high, watch for a shift toward Hardware Late-Cycle Squeeze (C)."
    ])
    ...
    watch_list.extend([
        "Watch for Demand staying above 60 to confirm ongoing ROI confidence.",
        "Watch for Stress staying below 60 to confirm lack of funding panic.",
        "Watch TSM and NVDA to ensure core leadership continues confirming semiconductor strength."
    ])
    ...
    watch_list.extend([
        "If Demand falls further, the divergence between hardware providers and their customers widens dangerously.",
        "If Stress continues to spike, the market may transition from a 'squeeze' into outright CAPEX Bubble Fear (D)."
    ])
    ...
    watch_list.extend([
        "Watch for any recovery in Breadth or Demand as early signs of stabilization.",
        "Watch Credit spreads (HYG/LQD) to see if AI stress is bleeding into broader macro liquidity."
    ])
...
for t in core_tickers:
    if t not in pivot_df.columns or pivot_df[t].dropna().empty:
        if t not in failed: failed.append(t)
...
elif ret_pct < 80 or not core_ok: status = "Failed"
elif ret_pct >= 95 and core_ok: status = "OK"
else: status = "Partial"
...
output = {
    ...
    "closest_regime": {"code": closest[0], "name": closest[1]},
...
hist_data = [d for d in hist_data if isinstance(d, dict) and d.get('date') != latest_date]
hist_data.append({
    "date": latest_date,
    "regime": regime,
    "regime_name": regime_name,
...
if csv_exists:
    try:
        with open(csv_file, 'r') as f:
            ...
            csv_rows = [row for row in reader if row.get('date') != latest_date]
    except Exception:
        csv_rows = []
...
print(f"Regime         : {regime} (Closest: {closest[0]} | Confidence: {confidence})")
print(f"Scores         : D={round(demand_score,1)}, B={round(bottleneck_score,1)}, R={round(substitution_score,1)}, S={round(stress_score,1)}, Br={round(breadth_score,1)}")
print(f"Files Written  : latest.json, history.json, score_history.csv")
