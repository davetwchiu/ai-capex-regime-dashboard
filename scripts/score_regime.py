...
substitution_score = normalize((sub_20 * 0.5) + (sub_60 * 0.5))

# 4. Composite Stress Score
# A. Underperformance (50%)
...
# Hard Threshold checking for 'True' hit
...
# Filter out today if it already exists (to overwrite)
...
with open(history_file, 'w') as f:
    json.dump(hist_data[-250:], f, indent=2) # Keep last year roughly
...
print(f"Update complete. Date: {latest_date} | Closest: {closest[0]} | Conf: {confidence}")
